...
const STATUS_PHASES = {
  ...
  benchmark: 'Reunindo referências e sinais de mercado...',
  ...
  tech: 'Entendendo a operação e tecnologia...',
  compliance: 'Verificando sinais de risco e conformidade...',
  rh: 'Identificando estrutura, liderança e decisores...',
...
function matchCategory(status: string): { key: StatusPhaseKey; extra?: string } | null {
  ...
  if (/^(Capturando intenção tática|Entendendo o objetivo da pergunta|Mapeando objetivo estratégico)/i.test(s))
    return { key: 'intent' };
  if (/^(Avaliando profundidade|Entendendo sua necessidade|Analisando complexidade)/i.test(s))
    return { key: 'complexity' };
  ...
  if (
    /^(Enriquecendo sinais e contexto comercial estratégico|Enriquecendo sinais e contexto comercial|Enriquecendo contexto comercial)/i.test(
      s,
    )
  )
    return { key: 'enrichment' };
  if (
    /^(Orquestrando protocolo de investigação forense|Orquestrando protocolo de análise|Montando protocolo de análise)/i.test(
      s,
    )
  )
    return { key: 'prompt' };
  if (/^(Infiltrando em fontes externas|Deep Research ativado|Sinais externos em análise)/i.test(s))
    return { key: 'deepResearch' };
  if (/^Buscando histórico de/i.test(s)) {
    const rawCompany = s
      .replace(/^Buscando histórico de\s*/i, '')
      .replace(/\.{0,3}\s*$/, '')
      .trim();
    return { key: 'deepResearch', extra: rawCompany };
  }
  if (/^(Auditando referências|Mapeando benchmarks|Cruzando referências de mercado|Reunindo referências)/i.test(s))
    return { key: 'benchmark' };
  if (/^(Consultando inteligência Senior|Consultando bases de conhecimento|Consultando inteligência interna)/i.test(s))
    return { key: 'knowledgeBase' };
  if (/^base RAG/i.test(s)) return { key: 'rag' };
  if (
    /^(Processando em motores de inferência tática|Consultando modelo analítico|Consultando modelo de IA|Processando no modelo)/i.test(
      s,
    )
  )
    return { key: 'model' };
  if (/^(Validando integridade|Validando consistência|Validando coerência|Validando achados)/i.test(s))
    return { key: 'validation' };
  if (/^(Sintetizando narrativa executiva de alto impacto|Sintetizando resposta executiva)/i.test(s))
    return { key: 'synthesis' };
  if (
    /^(Auditando consistência final|Revisando consistência final|Revisando entrega final|Auditando consistência final da entrega)/i.test(
      s,
    )
  )
    return { key: 'finalReview' };
  if (/^(Materializando recomendações práticas|Gerando resposta|Montando resposta prática)/i.test(s))
    return { key: 'response' };
  if (/^(Preparando ganchos para fechamento|Gerando ganchos|Preparando próximos passos)/i.test(s))
    return { key: 'hooks' };
  if (/^(Rastreando registros cadastrais|Consultando dados cadastrais|Buscando CNPJ|dados da empresa)/i.test(s))
    return { key: 'cadastral' };
  if (/^(Desconstruindo teia societária|Mapeando teia societária|sócios|grupos econômicos)/i.test(s))
    return { key: 'corporate' };
  if (
    /^(Analisando stack tecnológico|Analisando stack|stack tecnológico|sistemas utilizados|Investigando tech stack)/i.test(
      s,
    )
  )
    return { key: 'tech' };
  if (
    /^(Escaneando riscos fiscais|Verificando compliance|riscos fiscais|SINTEGRA|SEFAZ|Investigando riscos & compliance)/i.test(
      s,
    )
  )
    return { key: 'compliance' };
  if (/^(Mapeando centro de gravidade|Analisando RH|decisores|gestores|diretores)/i.test(s)) return { key: 'rh' };
  if (/^(Investigando malha logística|Investigando logística|supply chain|frota|armazenagem)/i.test(s))
    return { key: 'logistica' };
  if (/^(Auditando incentivos|Verificando incentivos fiscais|benefícios fiscais)/i.test(s)) return { key: 'fiscal' };
  if (
    /^(Mapeando inteligência territorial estratégica|Mapeando inteligência territorial|contexto regional|região|Investigando estratégia & expansão)/i.test(
      s,
    )
  )
    return { key: 'territorio' };
  if (/^(Calibrando Score PORTA|Calculando Score|PORTA score)/i.test(s)) return { key: 'scoring' };
  if (
    /^(Consolidando dossiê de inteligência final|Consolidando dossiê|dossiê final|relatório final|Finalizando dossiê modular)/i.test(
      s,
    )
  )
    return { key: 'consolidando' };
  if (
    /^(Mapeando ecossistema competitivo regional|Cruzando concorrentes|concorrentes regionais|mapeamento competitivo)/i.test(
      s,
    )
  )
    return { key: 'concorrentes' };
...
export function finalizeLoadingProgress(
  ...
  const shouldAppend = Boolean(normalized) && normalized !== 'Consolidando informações...';
  return {
    stage: '',
