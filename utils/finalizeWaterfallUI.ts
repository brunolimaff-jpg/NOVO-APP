...
export function finalizeWaterfallUI(params: FinalizeWaterfallUIParams): void {
  ...
  // 4. DOM cleanup: esconde overlay se React ainda não removeu
  if (typeof document !== 'undefined') {
    const stuckOverlay = document.querySelector<HTMLElement>('[data-testid="loading-smart-overlay"]');
    if (stuckOverlay) {
      stuckOverlay.style.display = 'none';
    }
  }
