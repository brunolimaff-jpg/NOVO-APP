...
1. **Count consistency** — Verify the same skill/agent/command/tool counts appear in: Root CLAUDE.md, Root README.md, docs/index.md, docs/getting-started.md, .claude-plugin/marketplace.json
2. **Path validation** — Verify all `source` paths in marketplace.json point to existing directories
3. **New script verification** — If new Python scripts were added, verify they run: `python3 path/to/new/script.py --help`
4. **Frontmatter check** — Verify all new SKILL.md, agent, and command files have valid YAML frontmatter

---

## Step 7: Summary report

Present a summary table to the user and ask if they want to commit and push the changes.
