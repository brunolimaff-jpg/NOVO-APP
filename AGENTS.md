...
- Agent role governance lives in `.agents/papeis/README.md` (canonical roles), `.agents/governanca/contrato-comunicacao-bruno.md` (communication contract), and `.agents/adaptadores/README.md` (tool adapters). Roles are not skills and are not listed in `skills-lock.json`.
