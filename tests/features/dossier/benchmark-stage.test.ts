...
describe('runDossierBenchmarkStage', () => {
  ...
  it('propaga abort como erro terminal', async () => {
    const abortError = new Error('request aborted');
    abortError.name = 'AbortError';
    getIsolatedBenchmarkMock.mockRejectedValue(abortError);
    ...
    await expect(
      runDossierBenchmarkStage({
        ...
        company: 'Acme Agro',
        signal: new AbortController().signal,
        appendWaterfallChunk: vi.fn(),
        optionalStepFailures: new Set<string>(),
