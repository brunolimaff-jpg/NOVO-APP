...
describe('runWithStepTimeout (produção — services/gemini/runtime)', () => {
  ...
  describe('abort', () => {
    it('abort pelo signal externo propaga para a action', async () => {
      ...
      const promise = runWithStepTimeout('step', actionThatRespectsAbort, controller.signal, 10_000);
      await vi.advanceTimersByTimeAsync(100);
      controller.abort();
    ...
    it('abort externo antes do timeout não confunde com TimeoutError', async () => {
      const controller = new AbortController();
      const promise = runWithStepTimeout('step', actionThatRespectsAbort, controller.signal, 2_000);
      await vi.advanceTimersByTimeAsync(500);
      controller.abort();
      await expect(promise).rejects.toThrow('Aborted');
