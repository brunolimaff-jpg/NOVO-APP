...
describe('api/gemini handler', () => {
  ...
  it('retorna 502 quando LiteLLM retorna vazio e fallback desativado', async () => {
    isLiteLLMEnabledMock.mockReturnValue(true);
    callLiteLLMMock.mockResolvedValueOnce({
      text: '   ',
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({
        _llm_error: true,
        _llm_error_reason: 'empty_response',
      }),
    );
  });

  it('retorna erro quando auth 401 e fallback desativado', async () => {
    isLiteLLMEnabledMock.mockReturnValue(true);
  ...
  it('retorna erro quando auth 403 (allowlist) e fallback desativado', async () => {
    isLiteLLMEnabledMock.mockReturnValue(true);
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({
        error: 'Operator not in LLM_ALLOWLIST',
      }),
    );
  });
