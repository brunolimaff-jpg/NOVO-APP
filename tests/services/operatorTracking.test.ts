...
const mockInsert = vi.hoisted(() => vi.fn(() => ({ then: (fn: () => void) => fn() })));
const mockUpdate = vi.hoisted(() => vi.fn(() => ({ eq: vi.fn(() => ({ then: (fn: () => void) => fn() })) })));
const mockUpsert = vi.hoisted(() => vi.fn(() => ({ then: (fn: () => void) => fn() })));
...
describe('operatorTracking', () => {
  beforeEach(() => {
    ...
    mockInsert.mockReturnValue({ then: (fn: () => void) => fn() });
    mockUpdate.mockReturnValue({ eq: vi.fn(() => ({ then: (fn: () => void) => fn() })) });
    mockUpsert.mockReturnValue({ then: (fn: () => void) => fn() });
  ...
  describe('trackOperatorEvent', () => {
    ...
    it('nao deve lancar excecao se o Supabase falhar', () => {
      mockInsert.mockReturnValue({
        then: (_onSuccess: () => void, onError?: () => void) => {
          onError?.();
  ...
  describe('startOperatorSession', () => {
    it('deve inserir sessao com ID gerado no cliente', () => {
    ...
    it('nao deve inserir quando Supabase indisponivel', () => {
      vi.mocked(isSupabaseAvailable).mockReturnValue(false);
  ...
  describe('initSessionTracking', () => {
    it('deve criar sessao e disparar app_opened', () => {
      initSessionTracking('op_test123', 'bruno@senior.com.br');
