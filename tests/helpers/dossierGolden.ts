...
export function validateDossierGolden(
  ...
  const orderedHeadings = [...dossierCase.requiredHeadings].sort((left, right) => {
    const leftIndex = matchableExpected.indexOf(normalizeForMatch(left));
    const rightIndex = matchableExpected.indexOf(normalizeForMatch(right));
