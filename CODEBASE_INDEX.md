...
- `FollowUpModal.tsx` — FollowUpModal, FollowUpScheduleResult
...
- `HelpCenterFloating.tsx` — HelpCenterFloating
- `InlineTypingResponse.tsx`
- `InstallPrompt.tsx` — default:InstallPrompt
...
- `ChatShell.tsx`
- `Composer.tsx`
- `contracts.ts` — RadarProps, ExtendedChatInterfaceProps, ChatTheme, StartInvestigationPayload
- `MessageTimeline.tsx`
...
- `message-helpers.ts`
- `message-orchestrator.ts` — useChatMessageOrchestrator, HandleSendMessageOptions, UseChatMessageOrchestratorOptions
...
- `useEmailModal.ts` — useEmailModal, EmailModalStatus
...
- `geminiService.ts` — Public facade preserved for App.tsx, ChatInterface.tsx, LoadingSmart.tsx and tests.
...
- `investigation-orchestration.ts` — sendMessageToGemini, generateDossierModule, getIsolatedBenchmark
- `porta.ts` — parsePortaFeeds, cleanPortaFeedMarkers, parseMarkers, ParsedPortaFeeds
- `recovery.ts` — debugRecovery, looksLikeMissedOpenQuestionAnswer, shouldRecoverOpenQuestionByJudge, trackOpenQuestionRecoveryAttempt
- `runtime.ts` — isMegaPromptRequest, isDeepDiveMessage, getDeepDiveSource, buildConversationHistory, buildTimeoutError, runWithStepTimeout, DeepDiveSource
- `sanitization.ts` — sanitizeStreamText, sanitizeHistoryText, isValidEmpresaParaBenchmark
- `sources.ts` — normalizeGroundingSources
...
- `intent.ts` — isOutOfScope, isProcessoAgricolaIntent, isIntegracaoIntent, hasFercusIntent, hasTalhaoIntent, hasGatecAgricolaIntent, hasBankingIntent, isBlockedIntent +2
- `prompting.ts` — getWarRoomSystemPrompt, buildWarRoomPrompt
- `query.ts` — queryWarRoom
- `retrieval.ts` — mergeDocContexts, filterDocsForProcessoAgricola, filterNoisyDocsContext, prioritizeBlocksByKeywords, loadWarRoomDocsContext
...
- `FooterCredits.test.tsx`
- `HelpCenterFloating.test.tsx`
...
- `geminiSources.test.ts`
- `investigation-orchestration.test.ts`
- `portaParser.test.ts`
...
- `sessionTitleHeuristics.test.ts`
- `socioRuralResearch.test.ts`
...
- `errorBoundaryAudit.ts` — generateUiErrorId, buildUiErrorReport, persistUiErrorAudit
- `errorHelpers.ts` — normalizeAppError, getFriendlyErrorMessage
- `featureAccess.ts` — getFeatureAccess, UserFeatureAccess
- `featureFlags.ts` — getFlag, FEATURE_FLAGS, FeatureFlagName
...
- `helpCenterGuardrails.ts` — normalizeHelpCenterQuestion, isHelpCenterAllowedQuestion, HelpCenterGuardrailResult
...
- `messageHelpers.ts` — isGenericCompanyLabel, pickCompanyLabel, resolveHintedCompany, ensureContinuitySuggestions
- `PDFGenerator.ts` — PDFGenerator — renderização programática com jsPDF (sem html2canvas) · PDFGenerator
- `porta.ts` — normalizePortaFlags, stripVisiblePortaFeedSections, getPortaCompatibility, stripPortaMarkers, calculatePortaScoreBruto, calculatePortaFlagMultiplier, buildPortaScoreFromFeeds, resolvePortaScore +2
- `printExport.ts` — renderMarkdownForPrint, buildPrintReportHtml, openPrintReportWindow, PrintReportOptions
- `privacy.ts` — maskCpf, sanitizeSensitivePersonalData, containsFullCpf
- `react-dom-shim.d.ts`
...
- `seniorEvidence.ts` — extractClienteSeniorData, buildSeniorEvidenceContext, appendSeniorEvidenceNote, enforceSeniorEvidenceConstraints
...
- `socioRuralResearch.ts` — extractPotentialSocioNames, buildSocioRuralSearchQueries, classifySocioRuralEvidence, buildSocioRuralInstructionContext, SocioRuralEvidenceStatus, SocioRuralEvidence
...
- `.agent/skills/playwright-skill/package.json`
- `.agent/skills/postgres-best-practices/metadata.json`
...
- `skills-lock.json`
- `tests/fixtures/dossier/scheffer-04733767000180/case.json`
- `tests/fixtures/dossier/scheffer-04733767000180/continuity-suggestions.json`
- `tests/fixtures/dossier/scheffer-04733767000180/lookup.json`
- `tsconfig.json`
- `vercel.json`
