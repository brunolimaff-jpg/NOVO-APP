...
export function buildComposePrompt(input: ComposeInput): string {
  ...
  return [
    ...
    'Linguagem executiva. 900-1500 palavras (NUNCA abaixo de 900 — o contrato rejeita abaixo; se ficar curto, aprofunde as seções 4-8). Máx 3 sinais.',
    ...
    'VISUAIS (EXPERIENCE-01C — decisão do Bruno 2026-08-10): NÃO escreva código Mermaid. Os 3 diagramas (Mapa do Caos na seção 2, Teia Societária na seção 3, Caminho da Venda na seção 9) são montados DEPOIS por um builder determinístico com a gramática visual padrão do Scout (graph LR + paleta core/satellite/danger/warning/neutral). Você fornece apenas o CONTEÚDO SEGURO que sustenta cada mapa: no texto da seção 2, liste os processos/operações confirmados (fatos do SAFE PACK) que o Mapa do Caos deve representar — operação principal e ramificações reais da cadeia, somente fatos Confirmados, sem gap/dor/ausência; na seção 3, liste a Tabela de CNPJs (whitelist do canonical) e a relação societária (same_root = mesma raiz; direct_pj_relation = relação PJ direta; partner_other_cnpj = relação lateral — JAMAIS chame lateral de "empresa do grupo", "controlada" ou "holding"); na seção 9, descreva o fluxo comercial conceitual: evidência segura → hipótese → discovery → decisão. Não insira nenhum bloco ```mermaid``` no texto: qualquer bloco Mermaid escrito por você será SUBSTITUÍDO pelo builder.',
