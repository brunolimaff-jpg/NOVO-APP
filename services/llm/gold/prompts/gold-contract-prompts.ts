...
export function buildComposePrompt(input: ComposeInput): string {
  ...
  return [
    ...
    'VISUAIS (EXPERIENCE-01C — decisão do Bruno 2026-08-10): NÃO escreva código Mermaid. Os 3 diagramas (Mapa do Caos na seção 2, Teia Societária na seção 3, Caminho da Venda na seção 9) são montados DEPOIS por um builder determinístico com a gramática visual padrão do Scout (graph LR + paleta core/satellite/danger/warning/neutral). Você fornece apenas o CONTEÚDO SEGURO que sustenta cada mapa: na seção 2, escreva uma leitura executiva da operação em 2-3 parágrafos curtos (6-8 frases), destacando a escala, o modelo de negócio e a posição competitiva — NÃO liste processos, elos ou fatos que o Mapa do Caos determinístico já representa; NÃO escreva "Mapa do Caos", "Elos da Cadeia" ou "Leitura Executiva" como rótulos — o builder injeta essas superfícies; na seção 3, inclua a Tabela de CNPJs (whitelist do canonical) e descreva a relação societária em LINGUAGEM HUMANA: "mesma raiz" (mesmo grupo raiz), "relação PJ direta" (sócia PJ direta na conta) ou "relação lateral" (demais CNPJs relacionados — JAMAIS chame lateral de "empresa do grupo", "controlada" ou "holding", e JAMAIS escreva "estrutura de holding"); a narrativa só afirma relação societária que exista no CANONICAL (matriz/directPjPartners) ou com status Confirmado no conteúdo seguro — se a Teia mostrar apenas a conta alvo, escreva "sem relações societárias confirmadas no conteúdo seguro" e NÃO afirme relação; na seção 9, escreva APENAS UMA frase curta de interpretação comercial e os 3 movimentos numerados — NÃO reconte em prosa o fluxo conceitual (evidência → hipótese → discovery → decisão) que o Mermaid do Caminho da Venda já representa: cada fato do fluxo tem UMA superfície visual; adicioná-lo em texto viola o single-owner; Não insira nenhum bloco ```mermaid``` no texto: qualquer bloco Mermaid escrito por você será SUBSTITUÍDO pelo builder. PROIBIDO na narrativa: NÃO escreva nenhum identificador técnico interno, rótulo entre parênteses com meta-instrução (ex.: "(Conteúdo para o Builder)", "(Operações Confirmadas)") nem identificador técnico de relação em snake_case — use única e exclusivamente a linguagem humana de relação/processo; qualquer metarótulo técnico na saída reprova o Gold.',
    ...
    `QSA (decisão do Bruno 2026-08-10 — Opção A): NUNCA liste pessoas do QSA individualmente, por extenso, em bullets, biografias ou Mermaid de atores. O QSA é cadastral, não um mapa de decisores. Na seção 3 (junto da Teia/Tabela de CNPJs) e na seção 5 PESSOAS-CHAVE, mostre APENAS o indicador agregado "👥 ${qsaCount} pessoas no QSA — papéis cadastrais, não decisores". Nomes só podem aparecer se houver papel funcional NÃO-QSA confirmado e material no conteúdo seguro — nunca liste QSA por padrão. BLOQUEIO DE GOVERNANÇA DERIVADA (BRU-119 follow-up, veredito Planejador): uma sócia PJ direta na Tabela de CNPJs NÃO autoriza a narrativa a rotulá-la de "holding", "controladora" nem a derivar mecanismo de governança, sponsor, fluxo decisório ou aprovação de investimentos (ex.: "governança é indicada pela estrutura de holding" ou "aprovação segue a governança da holding" são PROIBIDOS). Descreva APENAS o fato societário: "SCHEFFER PARTICIPAÇÕES S/A é sócia PJ direta"; qualquer especulação de governança/decisão fica como pergunta de discovery, não como afirmação.`,
    ...
    'EPISTEMOLOGIA DA AUSÊNCIA (BRU-119 follow-up, P0 — Planejador 2026-08-17): ausência não é confirmação. "Não consta", "não identificado", "não confirmado no CRM/recorte" JAMAIS recebe status ✅ Confirmado — nessas linhas use 🟠 A validar e escreva a ausência como ausência ("WMS/TMS não constam do portfólio contratado"). A expressão "ausência confirmada" (ou "✅ Confirmado (ausência)") é PROIBIDA em qualquer forma: tabela, texto, título, sinal, síntese. E ausência de módulo/tecnologia NUNCA gera Sinal, risco, nem hipótese de estoque físico, eficiência logística ou sistema paralelo — no máximo vira pergunta de discovery na seção 9 (ex.: "qual solução suporta a armazenagem?"). Sinal exige evidência POSITIVA observada no conteúdo seguro.',
