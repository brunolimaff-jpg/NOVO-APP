...
export function buildComposePrompt(input: ComposeInput): string {
  return [
    ...
    'VISUAIS (quando o conteúdo seguro permitir, inclua 1-2 diagramas Mermaid): (1) FLUXO DA OPERAÇÃO — etapas do processo (ex.: campo → beneficiamento → trading/exportação) mapeadas APENAS a partir de fatos do conteúdo seguro; (2) TEIA SOCIETÁRIA — entidades e CNPJs do grupo (alvo, holding, sócias, relacionadas). INCLUA AMBOS os diagramas (fluxo na seção 4 TECNOLOGIA ou 2 PERFIL; teia na seção 3 ESTRUTURA SOCIETÁRIA) sempre que os fatos do conteúdo seguro suportarem as etapas/entidades — ex.: se houver fatos confirmados de cultivo, beneficiamento e trading, o fluxo DEVE existir com essas etapas; se um diagrama não tiver suporte, omita-o em vez de inventar. Os diagramas DEVEM usar somente fatos do CANONICAL e do SAFE PACK — nunca invente etapa, empresa, CNPJ, relação ou seta; se um CNPJ/etapa não estiver no conteúdo seguro, omita-o em vez de preencher; dentro dos diagramas valem TODAS as regras de proveniência (termos sensíveis, "gap"/"lacuna" e ausência-virada-lacuna são proibidos também dentro dos mermaid).',
    'MATRIZ DE CNPJs: na seção 3 (ESTRUTURA SOCIETÁRIA), inclua uma tabela "Matriz de CNPJs" com colunas empresa | CNPJ | papel, listando APENAS os CNPJs presentes no CANONICAL e no SAFE PACK (alvo, holding, sócias diretas, relacionadas) — nunca invente CNPJ, nome ou papel; quando o CNPJ exato não constar do conteúdo seguro, escreva o papel sem o número.',
