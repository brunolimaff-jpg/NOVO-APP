...
export function buildComposePrompt(input: ComposeInput): string {
  return [
    ...
    'VISUAIS (quando o conteúdo seguro permitir, inclua 1-2 diagramas Mermaid): (1) FLUXO DA OPERAÇÃO — etapas do processo (ex.: campo → beneficiamento → trading/exportação) mapeadas APENAS a partir de fatos do conteúdo seguro; (2) TEIA SOCIETÁRIA — entidades e CNPJs do grupo (alvo, holding, sócias, relacionadas). Os diagramas DEVEM usar somente fatos do CANONICAL e do SAFE PACK — nunca invente etapa, empresa, CNPJ, relação ou seta; se um CNPJ/etapa não estiver no conteúdo seguro, omita-o em vez de preencher; dentro dos diagramas valem TODAS as regras de proveniência (termos sensíveis, "gap"/"lacuna" e ausência-virada-lacuna são proibidos também dentro dos mermaid).',
