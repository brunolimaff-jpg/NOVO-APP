...
export function buildComposePrompt(input: ComposeInput): string {
  return [
    ...
    'Linguagem executiva. 900-1500 palavras (NUNCA abaixo de 900 — o contrato rejeita abaixo; se ficar curto, aprofunde as seções 4-8). Exatamente 3 Mermaid com papéis FIXOS quando houver suporte: (1) Mapa do Caos Operacional na seção 2 PERFIL, (2) Teia Societária na seção 3 ESTRUTURA SOCIETÁRIA, (3) Caminho da Venda na seção 9 PRÓXIMOS PASSOS. Máx 3 sinais.',
    ...
    'VISUAIS: MAPA DO CAOS OPERACIONAL (obrigatório quando houver evidência segura suficiente) — produza um Mermaid executivo que mostre como as partes COMPROVADAS da operação se conectam: operação principal → ramificações reais da cadeia → sistemas/processos identificados → pontos de atenção sustentados (ex.: cultivo → beneficiamento → trading, com frota/armazenagem/sistemas como ramificações apenas quando comprovados). O diagrama deve ser compreensível em poucos segundos, mesmo sem ler o restante do Gold. Posição: peça visual principal da seção 2 PERFIL (ou 4 TECNOLOGIA quando o conteúdo for majoritariamente tecnológico). TEIA SOCIETÁRIA = 2º diagrama na seção 3 ESTRUTURA SOCIETÁRIA (junto da Tabela de CNPJs). CAMINHO DA VENDA = 3º diagrama na seção 9 PRÓXIMOS PASSOS. Regras: NÓ verdadeiro NÃO autoriza SETA inventada — toda aresta precisa de suporte no conteúdo seguro; ponto de atenção somente quando sustentado; ausência de tecnologia NUNCA vira gap; incerteza aparece como "não confirmado"/"a validar", nunca como fato; se um diagrama não tiver suporte, omita-o em vez de inventar; os diagramas DEVEM usar somente fatos do CANONICAL e do SAFE PACK; dentro dos diagramas valem TODAS as regras de proveniência (termos sensíveis, "gap"/"lacuna" e ausência-virada-lacuna são proibidos também dentro dos mermaid). LEGENDA E CORES: todo Mermaid usa classDef com cores distintas por categoria (azul = ✅ confirmado; verde = 🔗 relação/processo observado; amarelo = 🔎 a validar; cor comercial distinta = 🎯 próximo movimento) e cada diagrama fecha com uma legenda curta em texto ("Legenda: ...") explicando as categorias. A cor não é a única informação: toda categoria tem também texto e/ou ícone visível no nó.',
    ...
    'QSA (decisão do Bruno 2026-08-10 — Opção A): NUNCA liste pessoas do QSA individualmente, por extenso, em bullets, biografias ou Mermaid de atores. O QSA é cadastral, não um mapa de decisores. Na seção 3 (junto da Teia/Tabela de CNPJs) e na seção 5 PESSOAS-CHAVE, mostre APENAS um indicador agregado no formato "👥 N pessoas no QSA — papéis cadastrais, não decisores" (N = quantidade de pessoas do safePack.people com roleBasis qsa, ou do canonical.qsaPeople). Nomes só podem aparecer se houver papel funcional NÃO-QSA confirmado e material no conteúdo seguro — nunca liste QSA por padrão.',
