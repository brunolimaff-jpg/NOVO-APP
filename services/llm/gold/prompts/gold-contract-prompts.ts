...
export function buildComposePrompt(input: ComposeInput): string {
  // EXPERIENCE-01B: nomes QSA não são necessários para composição e não devem
  // atravessar para o Composer. Mantemos pessoas com papel funcional oficial ou
  // reportado; o QSA segue apenas como contagem agregada.
  const qsaPeople = input.safePack.people ?? [];
  const qsaCount = qsaPeople.filter(person => person.roleBasis === 'qsa').length;
