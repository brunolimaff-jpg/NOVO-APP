...
**Nota:** `EmailModal` e `FollowUpModal` já são *importados* (não inline). O problema é o **wiring de estado** (`showEmailModal`, handlers `onSend/onClose`, lógica de export) embutido diretamente em `App.tsx`.
...
> Sprint 13–16 planeja mover prompts de strings hardcoded para arquivos `.md`. Criar o golden test *depois* da migração só registra o estado novo, sem garantia de equivalência.
...
| 8 | Golden tests de prompts precisam ser criados *antes* da migração | Sprint 12 — Onda 4 adicionada |
