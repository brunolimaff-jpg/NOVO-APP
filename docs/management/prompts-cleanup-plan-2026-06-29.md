...
1. **1 caractere quebra markers `[[PORTA:*]]`** — o parser em `utils/porta.ts` usa regexes rígidas. Qualquer alteração em `prompts/mega/foundation.ts` linhas 774-860 (SHARED*PARSER_GUARD_BLOCK) ou em qualquer template `[[PORTA_FEED*\*]]` **QUEBRA O DOSSIÊ**.
...
| SHARED\_\* blocks (11 blocos em foundation.ts) copy-pasted em specialist prompts | ❌ NÃO — são corretamente importados via builders.ts                                                                 | —                                    |
...
1. **Criar tag `pre-prompts-cleanup`** (Princípio 12) — ✅ feito (PR #405)
2. **PR #1: H1** (deletar órfão `promptLeakShield.ts`) — ✅ feito (PR #405, commit `bb1030dc`)
3. **PR #2: H3** (adicionar 2 patterns de leak-shield) — ✅ feito (PR #405, commit `59fe4ab7`)
...
- **Se quebrar produção:** reverter o PR com `git revert <commit> && git push origin main` (nunca usar `--force` em main — repo com 20 usuários ativos)
