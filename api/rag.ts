...
async function embedQuery(ai: GoogleGenAI, query: string): Promise<number[] | undefined> {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const isDocsMode = namespace !== undefined;
    const docsNamespace = namespace?.trim();

    if (isDocsMode && (!docsNamespace || !ALLOWED_DOCS_NAMESPACES.has(docsNamespace))) {
    ...
    if (rawIndexName?.trim() && rawIndexName.trim() !== pineconeIndexName) {
      console.warn(
        `[${isDocsMode ? 'Docs RAG' : 'RAG'}] Invalid Pinecone index env "${rawIndexName}" detected. Falling back to "${pineconeIndexName}".`,
      );
    }
    ...
    if (isDocsMode) {
      ...
      const textBackedParts = filtered
      ...
      return res.status(200).json({
        context: textBackedParts.join('\n\n---\n\n'),
        matches,
      });
    }

    const globalNamespace = resolveOptionalNamespace(process.env.PINECONE_NAMESPACE);
    const queryTarget = globalNamespace ? index.namespace(globalNamespace) : index;
