...
export function WarRoomSidebar({ cfg, isOpen, onClose, onToggleSidebar, queryCount, t, accent }: WarRoomSidebarProps) {
  return (
    <>
      {isOpen && <div className="fixed inset-0 bg-black/50 z-40 sm:hidden" onClick={() => onToggleSidebar(false)} />}
      ...
      >
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className={`text-[10px] sm:text-[9px] uppercase tracking-widest font-semibold ${t.headerSub}`}>
                  Centro de Comando Tático
                </p>
              </div>
            </div>
            <button onClick={onClose} className={`p-1.5 rounded-lg transition-all text-xs ${t.closeTxt}`}>
              ✕
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 p-3 space-y-2 overflow-y-auto custom-scrollbar">
          <p className={`text-[10px] sm:text-[9px] font-bold uppercase tracking-[0.15em] ${t.labelTxt} mb-2`}>
            Modo ativo
          </p>
          <div
            className={`w-full text-left p-3 sm:p-3 rounded-xl border ${accent.bg[cfg.accent]} ${accent.border[cfg.accent]} shadow-sm`}
          >
