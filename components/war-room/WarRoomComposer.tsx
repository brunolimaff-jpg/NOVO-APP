...
export function WarRoomComposer({ cfg, input, inputRef, isLoading, onChange, onKeyDown, onSend, t, accent }: WarRoomComposerProps) {
  return (
    <div className={`p-3 sm:p-4 border-t ${t.terminalBdr} ${t.inputWrap}`}>
      <div className={`flex items-end gap-2 sm:gap-3 rounded-xl border ${accent.border[cfg.accent]} ${t.inputBg} p-2 transition-colors focus-within:shadow-sm`}>
        <textarea
          ref={inputRef}
          value={input}
