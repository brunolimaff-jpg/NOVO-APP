...
export const LoadingStepsList: React.FC<LoadingStepsListProps> = React.memo(
  ({
    ...
    <div className="flex min-w-0 flex-col">
      <h2
        className={`text-xs md:text-sm font-bold uppercase tracking-wider mb-3 ${
      ...
      <div className="flex flex-col gap-2.5 pr-0 md:gap-3 md:pr-2">
        {visiblePlannedStages.map((step, i) => {
          ...
          return (
            <div
              key={`${stepKey || 'step'}-${i}`}
              className={`flex items-center gap-3 ${isCompleted ? 'animate-fade-in' : ''}`}
            >
              {isCompleted ? (
        ...
        {shouldAppendCurrentStage ? (
          <div className="flex items-center gap-3">
            <StepSpinner isDarkMode={isDarkMode} />
            <span className={`text-sm flex-1 min-w-0 font-medium ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}>
              {currentRichLabel}
            </span>
            <StepTimer
              isDarkMode={isDarkMode}
