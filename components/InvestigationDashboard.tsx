...
import { getInvestigations, subscribe, type Investigation } from '../services/investigationStore';
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {(familias || []).slice(0, 4).map(f => (
        <span key={f} className="text-xs bg-gray-700 text-gray-300 rounded-full px-2.5 py-0.5">
          {f}
        </span>
      ))}
      {familias.length > 4 && <span className="text-xs text-gray-500">+{familias.length - 4}</span>}
    </div>
  );
}
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
      ...
      <p className="text-gray-500 text-xs max-w-xs">Pesquise uma empresa pelo chat para gerar seu primeiro Dossie.</p>
    </div>
  );
}
...
const InvestigationDashboard = React.memo(function InvestigationDashboard({
  ...
  const stats = useMemo(() => {
    const avgScore = all.length > 0 ? Math.round(all.reduce((acc, i) => acc + i.score, 0) / all.length) : 0;
    return {
      total: all.length,
      ...
      <div className="bg-gray-900 border-t md:border border-gray-700 rounded-t-2xl md:rounded-2xl w-full md:max-w-4xl h-[92dvh] md:max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="p-4 md:p-5 border-b border-gray-800 flex-shrink-0">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-lg md:text-xl font-bold text-white leading-tight">Meu Painel</h2>
              <p className="text-xs text-gray-400 mt-0.5">Empresas investigadas nesta sessao</p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="grid grid-cols-3 md:grid-cols-6 gap-2 md:gap-3 mb-4">
            ...
            <KPICard
              ...
              bgClass="bg-green-900/20 border border-green-900"
            />
            <KPICard
              value={stats.mornas}
              label="Mornas"
              valueClass="text-yellow-400"
              bgClass="bg-yellow-900/20 border border-yellow-900"
            />
            <KPICard
              value={stats.frias}
              label="Frias"
              valueClass="text-red-400"
              bgClass="bg-red-900/20 border border-red-900"
            />
            <KPICard
              value={stats.clientes}
              label="Clientes"
              valueClass="text-blue-400"
              bgClass="bg-blue-900/20 border border-blue-900"
            />
            <KPICard
              value={stats.avgScore}
              label="Avg Score"
              valueClass={scoreColor(stats.avgScore)}
              bgClass="bg-gray-800"
            />
          </div>
        ...
        {/* LISTA */}
        <div
          className="overflow-y-auto flex-1 p-4 md:p-5 space-y-2.5"
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex-shrink-0 flex flex-col items-end gap-1">
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-medium ${inv.isCliente ? 'bg-blue-900/30 text-blue-400' : 'bg-gray-700 text-gray-400'}`}
                    >
                      {inv.isCliente ? 'Cliente' : 'Prospect'}
                    </span>
                    {inv.gaps.length > 0 && (
                      <span className="text-xs text-yellow-500/80 font-medium">{inv.gaps.length} gaps</span>
                    )}
                  </div>
                </div>
                ...
                {/* Resumo */}
                {inv.resumo && <p className="text-gray-400 text-xs mt-2 line-clamp-2 leading-relaxed">{inv.resumo}</p>}
              </div>
            );
          })}
        </div>
        ...
        {filtered.length > 0 && (
          <div
            className="px-4 md:px-5 py-3 border-t border-gray-800 flex-shrink-0 flex justify-between items-center"
            style={{ paddingBottom: 'max(0.75rem, env(safe-area-inset-bottom))' }}
          >
            <span className="text-xs text-gray-500">
              {filtered.length} de {stats.total} empresas
            </span>
            <span className="text-xs text-gray-600">Clique em uma empresa para reabrir o Dossie</span>
          </div>
        )}
      </div>
    </div>
