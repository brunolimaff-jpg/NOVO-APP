...
const RadarBell: React.FC<RadarBellProps> = ({ unreadCount, isScanning, onClick, isDarkMode }) => {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`relative flex items-center gap-1 rounded-lg px-2 py-2 transition-colors duration-200 ${colorClasses}`}
      title={isScanning ? 'Radar varrendo...' : `Radar Setorial${hasUnread ? ` (${unreadCount} novas)` : ''}`}
    >
      {/* Sino: outline quando vazio, filled quando há notificações */}
      {hasUnread ? (
        /* Sino cheio (filled) */
        <svg
          className={`w-5 h-5 flex-none ${hasUnread ? 'animate-bell-shake' : ''}`}
          viewBox="0 0 24 24"
          fill="currentColor"
          aria-hidden="true"
        >
