    ...
    () =>
      sessions
        .filter(session => {
          ...
          return empresa.includes(term) || cnpj.includes(term) || titulo.includes(term) || display.includes(term);
        })
      ...
      >
        <div className="w-72 flex flex-col h-full min-w-[18rem]">
                ...
                {filteredSessions.map(session => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <div className={`flex-none text-lg opacity-80 mt-0.5`}>🏢</div>
                        <div className="flex-1 min-w-0 pr-12">
                          <h3
                            className={`text-sm font-medium truncate ${isActive ? 'text-emerald-500' : theme.textPrimary}`}
                          >
                            {displayName}
                          </h3>
                          <div className="flex items-center justify-between gap-2 mt-0.5">
                            ...
                            {session.scoreOportunidade && (
                              <span
                                className={`text-[9px] px-1 rounded bg-emerald-500/10 text-emerald-500 border border-emerald-500/20`}
                              >
                          ...
                          {messagePreview && (
                            <p className={`text-xs mt-1 line-clamp-1 ${theme.textSecondary}`}>{messagePreview}</p>
                          )}
                        </div>
                      </button>
                      ...
                      <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1 transition-opacity md:opacity-0 md:group-hover:opacity-100 z-10">
                        <ConfirmPopover message="Excluir dossiê?" onConfirm={() => onDeleteSession(session.id)}>
                          {({ onClick }) => (
          ...
          <div className={`p-4 border-t flex-none ${isDarkMode ? 'border-slate-800' : 'border-slate-200'}`}>
            <p className={`text-[10px] text-center ${theme.textSecondary}`}>🦅 Senior Scout 360</p>
          </div>
        </div>
      </aside>
    </>
