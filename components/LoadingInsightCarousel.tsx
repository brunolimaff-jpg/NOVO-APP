...
export const LoadingInsightCarousel: React.FC<LoadingInsightCarouselProps> = React.memo(
  ({
    ...
    const dotStart =
      totalCuriosities <= MAX_DOTS ? 0 : Math.max(0, Math.min(activeInsightIndex - 2, totalCuriosities - MAX_DOTS));

    return (
      <div
        className={`flex-shrink-0 px-4 py-3 md:px-8 md:py-4 border-t ${
          isDarkMode ? 'border-slate-800' : 'border-slate-200'
        }`}
      >
        <div
          className={`mx-auto w-full max-w-3xl rounded-xl px-4 py-3 md:px-5 ${
        ...
        >
          <div className="flex items-start gap-3 mb-2">
            ...
            <div
              className={`flex-1 min-w-0 transition-opacity duration-300 ${isFadingOut ? 'opacity-0' : 'opacity-100'}`}
            >
              <p
              ...
              <div className="max-h-none">
                <p
                  className={`text-sm font-medium leading-relaxed ${isDarkMode ? 'text-slate-100' : 'text-slate-700'}`}
                >
