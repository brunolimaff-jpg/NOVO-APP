...
const SystemHealthCheck: React.FC<SystemHealthCheckProps> = ({ isDarkMode, onClose }) => {
  ...
  const updateTest = (name: string, updates: Partial<TestResult>) => {
    setResults(prev => prev.map(test => (test.name === name ? { ...test, ...updates } : test)));
  };
  ...
  const runTests = async () => {
    ...
    try {
      ...
      if (!resultado.failed && resultado.context.trim().length > 0) {
        ...
        updateTest('🧠 War Room - Base Interna (opcional)', {
          status: 'error',
          message: `${resultado.failed ? 'Falha operacional' : 'Vazio'} (${duration}ms)`,
        });
      }
    } catch (error: unknown) {
      updateTest('🧠 War Room - Base Interna (opcional)', {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  Verificando todos os componentes críticos
                </p>
              </div>
            </div>
            <button
          ...
          {results.length === 0 ? (
            <div className="text-center py-12">
              <p className={`text-sm mb-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                Clique no botão abaixo para iniciar o diagnóstico completo
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="space-y-2">
                {results.map(test => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className={`text-xl ${getStatusColor(test.status)}`}>{getStatusIcon(test.status)}</span>
                        <div>
